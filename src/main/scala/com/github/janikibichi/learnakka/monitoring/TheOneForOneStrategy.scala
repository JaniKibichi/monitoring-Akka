...
object TheOneForOneStrategy extends App{
  val actorSystem = ActorSystem("SuperVisor")

  //Create and define an Actor in the ActorSystem
  val startStrategy = actorSystem.actorOf(Props[SupervisorStrategy])

  //Send a message to the Actors
  startStrategy ! "Start"
}
...
//Create a supervisor Actor that defines the OneForOne Strategy for its kids
